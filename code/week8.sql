...
SELECT 신청날짜, 
	...
	TO_CHAR(신청날짜, 'YYYY년"MM"월"DD"일"') AS 형식2
FROM 수강;

-- 저장 프로시저

CREATE OR REPLACE PROCEDURE Insert0rUpdateCourse(
...
--새 과목 추가하기
CALL Insert0rUpdateCourse('c006','연극학개론','310','교양힉부',2);
SELECT * FROM 과목;

-- 과목 업데이트 하기
CALL Insert0rUpdateCourse('c006','연극학개론','410','교양힉부',1);
SELECT * FROM 과목;

-- BestScore 프로시저
CREATE OR REPLACE PROCEDURE SelectAverage0fBestScore(
	IN Score INT,
	OUT Count INT
)
LANGUAGE plpgsql
...
DECLARE Count INT;
BEGIN
	CALL SelectAverage0fBestScore(95,Count);
