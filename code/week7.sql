-- 새로운 사본 테이블 만들어요
CREATE TABLE 과목2
    (과목번호 CHAR(4) NOT NULL PRIMARY KEY,--모든 과목 번호는 4자리까지 제한
...
ALTER DATABASE univdb OWNER TO manager; --postgres계정만 실행 할 수 잇음
...
CREATE VIEW V2_과목수강현황(과목,강의실,수강인원수) AS 
...
--인덱스
CREATE INDEX idx_수강 ON 수강(학번,과목번호); --오류: 소유자만 가능 postgres

CREATE UNIQUE INDEX idx_과목 ON 과목 (이름 ASC);

CREATE UNIQUE INDEX idx_학생 ON 학생 (학번);

SHOW INDEX FROM 학생; --MYSQL