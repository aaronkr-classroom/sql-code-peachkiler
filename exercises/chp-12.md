...
**01 빅데이터가 큰 이슈로 부각된 이유로 거리가 면 것은?** 답: 3번
...
**02 데이터 과학(data science) 에 관한 설명으로 거리가 먼 것은?** 답: 1번
...
**03 데이터 과학(data science) 에 관런된 기술로 가장 거리가 먼 것은?** 답: 4번
...
**04 음성, 동영상, 블로그, SNS 메시지 등과 같이 정해진 형식이나 구조가 없는 정형화되지않은 데이터를 의미하는 것은?** 답: 1번
...
**05 반정형 데이터 (semi-structured data) 에 속하지 않는 것은?** 답: 2번
...
**06 텍스트 데이터 수집 기술에 해당하지 않는 것은?** 답: 4번
...
**07 데이터 정제(data cleansing) 대상에 해당하지 않는 것은?** 답: 3번
...
**08 빅데이터의 주요 특성에 해당하지 않는 것은?** 답: 4번
...
**09 대용량 데이터의 분산 병렬처리를 위해 구글에서 만든 소프트웨어 프레임워크는?** 답: 1번
...
**11 NoSOL 데이터베이스의 장점이 아닌 것은?** 답: 3번
...
**12 NoSOL 데이터베이스의 대표적인 4가지 유형에 해당하지 않는 것은?** 답: 2번
...
**13 군집 (clustering) 분석에 관한 설명으로 옳은 것은?** 답: 4번
...
**14 오피니언 마이닝 (opinion mining)에 관한 설명으로 옳은 것은?** 답: 2번
...
**15 NoSQL에 관한 설명으로 옳지 않은 것은?** 답: 2번
...
2. 데이터웨어하우스, 데이터마트나 데이터마이닝에서는 수집된 데이터를 분석이 용이하도록 통일된 형태로 변환하는 과정을 (정형화)이라고 한다.
...
5. (데이터마이닝)은 데이터 집합으로부터 의미있는 통계적 규칙이나 패턴을 찾아냄으로써 데이터의 새로운 가치를 찾아내는 기법이다.
...
SQL DB: 관계형 데이터베이스, 테이블 형식, 변경 어려운 고정된 스키마(정적), 고비용, 빅데이터 처리에 한계, 분산 처리 어려움, 외래키 조인 방식 , 소용량 핵심 비즈니스 데이터 처리 최적화

NoSQL DB: 비관계형 데이터베이스, 문서 키값 컬럼패밀리 그래프 형식, 스키마가 필요없는 유연한 스키마(동적), 저비용, 클라우드 컴퓨팅 빅데이터 처리에 효과적, 분산 처리 쉬움, 참조(링크) 내장(임베디드) 방식, 대용량 소셜미디어 로그 iot 데이터 처리 최적화
