...
**01 다음 SOL 문장 중 문법적으로 옳은 것은?** 답: 1번
...
**03 SOL의 SELECT절에 사용할 수 없는 키워드는?** 답: 2번
...
**04 다음 SOL문에 대한설명으로옳지 않은것은?** 답: 4번
...
**05 다음 SOL문 중 문법적으로 옳지 않은 것은?** 답: 4번
...
**06 네 이터 베 이스에셔 널 (NULL) 값에 대 한 설명 중 옳지 않은 것은?** 답: 3번
...
**07 다음 중 SELECT 문의 질의를 계산하는 순서로 맞는 것은?** 답: 2번
...
**08 SO L의 GROUP BY 에 대한 설명으로 옳지 않은 것은?** 답: 4번번
...
1. 관계형 데이터베이스 표준 언어인 SQL은 (__Structured Query Language___)의 약어이다.
2. SQL은 대화식 그리고 (__배치___)의 2가지 방식으로 사용할 수 있다.
3. univDB로 작업 대상 데이터베이스를 변경하는 명령은 ‘(__USE___) univDB'이다.
4. SELECT절의 ‘(__asterisk___)’는 데이블의 모든 열 이름을 나열한 것과 같다.
...
FROM 학생
WHERE 성별 = 'M'
AND 나이 = (SELECT MIN(나이) FROM 학생 WHERE 성별 = 'M');
...
SELECT 수강.학번, 수강.과목번호, (중간성적 + 기말성적) AS 총점
FROM 수강;
...
SELECT 평가학점, COUNT(DISTINCT 학번) AS 학생수
FROM 수강
GROUP BY 평가학점;
